...
namespace Program
{
    internal class Point2D: Point
    {
        ...
        public override float cal_distance(Point2D a_point, Point2D b_point)
        {
    ...
    class Alpha
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i <= 7; i++)
            {
                float x = (float)rnd.Next(1, 100);
                float y = (float)rnd.Next(1, 100);
            ...
            {
                Console.WriteLine("\nPoints that formed a right-triangle: ");
                foreach((string,string,string) item in right_triangle)
                {
                    string[] tree_points = {item.Item1, item.Item2, item.Item3};

                    foreach(string p in tree_points)
                    {
                        Console.WriteLine($"{p}: {distances[p]}");
                    }
                    Console.WriteLine();
                }
            }
        ...
        public static void Display_distances(Dictionary<string, float> distances)
        {
            Console.WriteLine("\n Distances:");
